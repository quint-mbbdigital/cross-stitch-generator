...
class TestTextureDetectionFixtures:
    ...
    def create_solid_color_image(self) -> Image.Image:
        ...
        image_array = np.full(
            (100, 100, 3), [128, 128, 128], dtype=np.uint8
        )  # Solid gray
    ...
    def create_halftone_pattern_image(self) -> Image.Image:
        ...
        image_array = np.full(
            (100, 100, 3), [200, 200, 200], dtype=np.uint8
        )  # Light gray base
                    ...
                    for dx in range(-3, 4):
                        if 0 <= center_y + dy < 100 and 0 <= center_x + dx < 100:
                            ...
                            if distance <= 2.5:  # Create circular dots
                                # Make dots darker
                                image_array[center_y + dy, center_x + dx] = [
                                    160,
                                    160,
                                    160,
                                ]
...
class TestTextureDetector:
    ...
    def test_solid_color_image_no_texture_warning(self, tmp_path):
        ...
        assert not result.has_problematic_texture
    ...
    def test_fabric_texture_triggers_warning(self, tmp_path):
        ...
        assert isinstance(result, TextureWarning)
        assert result.has_problematic_texture
        assert (
            "texture" in result.warning_message.lower()
            or "background" in result.warning_message.lower()
        )
...
class TestPatternGeneratorTextureIntegration:
    ...
    def test_cli_integration_texture_warning_displayed(self, tmp_path):
        ...
        assert output_path.exists(), (
            "Output file should be created despite texture warning"
        )

        # Verify texture warning appears in stderr
        stderr_output = captured_stderr.getvalue()
        assert "WARNING:" in stderr_output, (
            f"Expected WARNING in stderr, got: {stderr_output}"
        )
        assert (
            "texture" in stderr_output.lower() or "background" in stderr_output.lower()
        ...
        assert any(
            ...
            for keyword in ["preprocess", "blur", "max-colors", "smooth", "background"]
