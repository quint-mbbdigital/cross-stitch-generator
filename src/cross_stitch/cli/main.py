...
from ..utils import CrossStitchError
...
def info_command(args) -> int:
    ...
    try:
        ...
        for name, target in info["target_resolutions"].items():
            ...
            print(
                f"  {name}: {size[0]}x{size[1]} (scale: {scale_x:.2f}x, {scale_y:.2f}x)"
            )
    ...
    try:
        ...
        if config.check_for_texture:
            ...
            if texture_result.has_problematic_texture:
                ...
                print(
                    "Processing will continue, but results may be suboptimal.",
                    file=sys.stderr,
                )
        ...
        if not args.quiet:
            ...
            for name, pattern in pattern_set.patterns.items():
                print(
                    f"  {name}: {pattern.width}x{pattern.height} stitches, {pattern.unique_colors_used} colors"
                )
...
Supported image formats: PNG, JPG, JPEG, GIF, BMP, TIFF, WEBP
    ...
    gen_parser = subparsers.add_parser(
        "generate", help="Generate cross-stitch patterns"
    )
    ...
    gen_parser.add_argument(
        "--max-colors", "-c", type=int, help="Maximum number of colors (default: 256)"
    )
    gen_parser.add_argument(
        "--quantization",
        "-q",
        choices=["median_cut", "kmeans"],
    ...
    def validate_percent(value):
        ...
        if fvalue < 0.0 or fvalue > 100.0:
            raise argparse.ArgumentTypeError(
                f"min-color-percent must be between 0.0 and 100.0, got {fvalue}"
            )
        return fvalue
    ...
    gen_parser.add_argument(
        "--cell-size", type=float, help="Excel cell size in points (default: 20.0)"
    )
    gen_parser.add_argument(
        "--no-legend", action="store_true", help="Do not include color legend sheet"
    )
    gen_parser.add_argument(
        "--legend-name",
        type=str,
    ...
    gen_parser.add_argument(
        "--no-dmc", action="store_true", help="Explicitly disable DMC color matching"
    )
    gen_parser.add_argument(
        "--dmc-database",
        type=str,
    ...
    info_parser.add_argument(
        "--estimate-time", action="store_true", help="Include processing time estimates"
    )

    # Add the same configuration options to info command (for analysis)
    info_parser.add_argument(
        "--resolutions", "-r", type=str, help="Target resolutions for analysis"
    )
    info_parser.add_argument(
        "--max-colors", "-c", type=int, help="Maximum number of colors for analysis"
    )
    info_parser.add_argument(
        "--quantization",
        choices=["median_cut", "kmeans"],
