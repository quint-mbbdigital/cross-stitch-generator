    ...
    try:
        ...
        if not path.exists():
            raise ValidationError(
                "Image file does not exist", field="image_path", value=str(path)
            )

        # Check if it's a file (not directory)
        if not path.is_file():
            raise ValidationError(
                "Path is not a file", field="image_path", value=str(path)
            )
        ...
        if not os.access(path, os.R_OK):
            raise ValidationError(
                "Image file is not readable", field="file_permissions", value=str(path)
            )
        ...
        raise ValidationError(
            f"Image validation failed: {e}", field="image_file", value=str(image_path)
        )
...
def _validate_resolutions(resolutions: List[Tuple[int, int]]) -> None:
    ...
    if not resolutions:
        raise ValidationError(
            "At least one resolution must be specified", field="resolutions"
        )
...
def _validate_excel_settings(config: GeneratorConfig) -> None:
    ...
    if (
        not config.output_filename_template
        or "{base_name}" not in config.output_filename_template
    ):
        raise ValidationError(
...
def validate_resolution_for_image(
    image_path: Union[str, Path], resolution: Tuple[int, int]
) -> None:
    """
